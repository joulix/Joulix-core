...
describe("CarbonLedgerGoO", function () {
  ...
  it("mint → retire: poprawny balans po umorzeniu", async () => {
    ...
    await expect(
      goo.connect(issuer).mint(user.address, 1, ethers.parseEther("10"), "0x")
    ).to.emit(goo, "TransferSingle");

    // user umarza 2e18
    await expect(
      goo.connect(user).retire(1, ethers.parseEther("2"), "retire test")
    ).to.emit(goo, "Retired");
  ...
  it("revoke blokuje transfery danego tokenId", async () => {
    ...
    await expect(goo.connect(admin).setRevoked(1, true, "bad batch"))
      .to.not.be.reverted;
  ...
  it("role: tylko ISSUER może mintować", async () => {
    ...
    // OK: issuer mintuje
    await expect(
      goo.connect(issuer).mint(user.address, 3, 1n, "0x")
    ).to.emit(goo, "TransferSingle");

    // NIE OK: losowy user próbuje mintować
    await expect(
      goo.connect(user).mint(user.address, 3, 1n, "0x")
    ).to.be.revertedWithCustomError(goo, "AccessControlUnauthorizedAccount");
  });
  ...
  it("retireBatch umarza wiele tokenId i aktualizuje salda", async () => {
    ...
    await expect(
      goo.connect(user).retireBatch([11, 22], [40n, 50n], "batch retire")
    ).to.not.be.reverted;
