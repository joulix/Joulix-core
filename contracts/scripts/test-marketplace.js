...
async function main() {
  ...
  // 1) Anuluj stary listing (jeśli istnieje)
  console.log("\n🗑️ Canceling old listing...");
  try {
    const nextId = await MarketAdmin.nextId();
    const OLD_ID = nextId - 1n;
    const remaining = await MarketAdmin.remaining(OLD_ID);
    
    if (remaining > 0n) {
      console.log(`  Found listing ${OLD_ID} with ${remaining} remaining`);
      const tx = await MarketAdmin.cancel(OLD_ID);
      await tx.wait();
      console.log("  ✅ Old listing canceled");
    } else {
      console.log("  No active listing found");
    }
  } catch (error) {
    console.log("  ⚠️ No listing to cancel or error:", error.message);
  }
