...
contract CarbonLedgerGoO is ERC1155, ERC1155Supply, AccessControl, Pausable {
    ...
    /// Limit długości uzasadnienia (gaz/DoS)
    ...
    function retire(uint256 id, uint256 value, string calldata reason) external {
        ...
        require(rb.length > 0, "reason empty");
        require(rb.length <= MAX_REASON_BYTES, "reason too long");
        require(balanceOf(msg.sender, id) >= value, "insufficient balance");
    ...
    function retireBatch(
        ...
        // _burnBatch zrevertuje jeśli gdziekolwiek brakuje balansu
